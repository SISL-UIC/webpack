	...
	apply(parser) {
		...
		parser.hooks.new.for("URL").tap(PLUGIN_NAME, (_expr) => {
			...
			if ((request = evaluatedExpr.asString())) {
				...
				/** @type {import("../Entrypoint").EntryOptions | undefined} */
				let entryOptions;
				if (importOptions && importOptions.webpackEntryOptions !== undefined) {
					if (
						...
						entryOptions = /** @type {import("../Entrypoint").EntryOptions} */ (
